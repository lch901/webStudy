...
insert into t_admin(i_admin,mid,mpw,nm)values(1,'admin','1212','°ü¸®ÀÚ');
...
insert into t_product(i_product,nm,price,pic)values((select nvl(max(i_product),0)+1 from t_product),'¹ä¸Ô´Â µà°ø',1000000,'https://www.bloter.net/wp-content/uploads/2016/11/dugong-765x574.jpg');
...
select * from t_product where i_product between 1 and 20 and nm = '¹ä¸Ô´Â µà°ø';
select * from t_product where i_product between 1 and 20 and nm='%¹ä%';
select i_product,nm,price,pic,qty,yn_sale,info,rank() over(order by i_product) as num from t_product where num = 11;--º¸Åëselect¹®¿¡ num°ªÀº where¹® »ç¿ëºÒ°¡´É
select * from 
(select i_product,nm,price,pic,qty,yn_sale,info,rank() over(order by i_product) as num 
from t_product where nm like '%µà%') 
where num between 1 and 5;--¼­ºêÄõ¸®¾È¿¡ num°ªÀº where »ç¿ë°¡´É
